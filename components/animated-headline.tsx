...
const lineOne = ["Engineering", "Premium", "Front-Ends"];
const lineTwo = ["for", "Complex", "Product", "Systems"];
...
export function AnimatedHeadline() {
    ...
    return (
        <h1 className="mt-7 max-w-5xl text-balance text-[2.25rem] md:text-[3rem] font-semibold leading-[0.88] tracking-[-0.075em] text-white sm:leading-[0.86] lg:max-w-6xl">
            <span className="sr-only">Engineering premium front-ends for complex product systems.</span>
            <span
                aria-hidden="true"
                className="inline-flex justify-center md:justify-start flex-wrap gap-x-3 gap-y-2 sm:gap-x-4 md:gap-y-3"
            >
                {words.map((word, index) => {
                    const isGradient = word === "Premium" || word === "Front-Ends";
